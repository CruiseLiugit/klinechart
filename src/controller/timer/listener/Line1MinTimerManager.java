...
import controller.timer.task.Line1MinTimerTask;
 ...
 *
 */
public class Line1MinTimerManager implements ServletContextListener {
	...
	public void contextInitialized(ServletContextEvent event) {
		 //定义定时器
		  timer = new Timer("1分钟分时图定时器",true);
		  //启动备份任务,每月(4个星期)执行一次
		  ...
		  //1分种执行一次
		  timer.schedule(new Line1MinTimerTask(servletContext),DateUtils.NO_DELAY, DateUtils.MILLIS_IN_MINUTE);
	}
